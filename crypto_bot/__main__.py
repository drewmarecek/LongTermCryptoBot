...
class TradingBot:
    ...
    def run_backtest(self, df: pd.DataFrame, symbol: str) -> None:
        ...
        for i in range(warmup, len(enriched)):
            ...
            if self._open is not None:
                self._maybe_exit_long(symbol, bar, ts_iso, float(bar["low"]), float(bar["high"]))
                continue
    ...
    def _try_open_long(self, symbol: str, bar: pd.Series, entry_time_iso: str) -> None:
        ...
        logger.info(
            "[%s] BUY %s @ %.6f | SL %.6f TP %.6f qty %.8f (risk $%.2f)",
            entry_time_iso,
            symbol,
            bracket.entry_price,
            bracket.stop_loss,
    ...
    def _maybe_exit_long(
        ...
        op = self._open
        sl, tp = op.stop_loss, op.take_profit
...
def main() -> int:
    ...
    if data_source == "csv":
        csv_path = os.environ.get("CRYPTO_BOT_CSV_PATH", config.DEFAULT_CSV_PATH)
        sym_csv = os.environ.get("CRYPTO_BOT_SYMBOL", "").strip()
        if not sym_csv or sym_csv.lower() == "all":
            sym_csv = DEFAULT_SYMBOLS[0]
            logger.info(
                "CSV mode: defaulting CRYPTO_BOT_SYMBOL to %s for trade labels.",
                sym_csv,
            )

        try:
            raw = load_ohlcv_from_csv(csv_path)
        except (OSError, ValueError) as exc:
            logger.error("CSV load failed: %s", exc)
            return 4

        logger.info(
            "CSV mode: using full series (%s rows); CRYPTO_BOT_TARGET_BARS ignored.",
            len(raw),
        )

        logger.info(
            "CSV backtest path=%s symbol=%s rows=%s timeframe label=%s (offline)",
            csv_path,
            sym_csv,
            len(raw),
            timeframe,
        ...
        logger.info("Finished backtest for %s | balance $%.2f", sym_csv, risk.balance)
